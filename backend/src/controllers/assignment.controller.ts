...
import { bulkAssignFundingRound, createAssignment, listApplicationAssignments, listMyAssignments, removeAssignment, updateDueAt } from "../services/assignment.service.js";
import { readAssignmentInput, readDueAt } from "../utils/assignmentValidation.js";
import { readBulkAssignmentInput } from "../utils/discoveryValidation.js";
...
export const bulk: RequestHandler = async (req,res,next) => { try { res.json({ results: await bulkAssignFundingRound(id(req.params.fundingRoundId, "funding round ID"), readBulkAssignmentInput(req.body), actor(req)) }); } catch(e){next(e);} };
